...
For the complete itemised list of what was added and removed, see the [changelog](https://github.com/CloudCannon/rcc/blob/v2/CHANGELOG.md). The steps below cover everything you need to *do*.

Two notes before you start:

- **Machine translation.** v2 bundles no translation service. Because everything now lives in one JSON format, plugging one in is a standalone postbuild step that reads and writes `rosey/locales/*.json` — see [External Integrations](/docs/integrations/). Smartling users will need to port their integration, but it's a smaller job than v1's multi-step chain.
- **Staging sites.** If you ran the v1 staging-to-production split, you can decommission the staging site once the migration is verified.
...
Place this inside the `<body>`, before or after your `<main>` element. See [Getting Started: Step 2](/docs/#step-2-import-the-script-in-your-layout) for more detail.
...
No build configuration is needed for this on any generator — `install-client` runs after your site build and copies the client into the output directory. See [SSG Setup](/docs/ssg-setup/) for the layout snippet in each generator's template language.
...
A ready-to-adapt sample lives in this repo at [`scripts/remap-locale-keys.mjs`](https://github.com/CloudCannon/rcc/blob/v2/scripts/remap-locale-keys.mjs). It matches new keys to old translations by source text; copy it into your site and tweak the matching rule if your migration is fuzzier. Pass `--dry-run` to preview before writing.
...
If the switcher doesn't appear, check the [troubleshooting guide](/docs/known-issues/#troubleshooting).
