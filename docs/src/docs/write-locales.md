---
_schema: docs
permalink: /docs/write-locales/
title: "write-locales CLI"
layout: layouts/page.html
eleventyNavigation:
  key: "write-locales CLI"
  order: 2
tags: "reference"
SEO_options:
  title:
...
| Flag | Description |
| --- | --- |
| `-s, --source <dir>` | Rosey directory containing `base.json` (default: the parent of the config's `locales` directory, else `rosey`) |
| `-l, --locales <codes>` | Comma-separated locale codes (e.g. `fr,de,es`); falls back to the config's `languages`, else auto-detects from existing files |
| `-d, --dest <dir>` | Build output directory; writes a locale manifest to `{dest}/_rcc/locales.json` (default: the config's `source`) |
| `--keep-unused` | Preserve locale keys that are no longer in `base.json` instead of removing them. Used when remapping translations onto new keys — see [Migrating from v1](/docs/migration-from-v1/) |
| `-h, --help` | Show help |
...
| `keepUnused` | `boolean` | Preserve keys not in `base.json` instead of deleting them (default: `false`) |
...
3. **Each entry should have a `_base_original` field** if you want [stale translation detection](/docs/stale-translations/). This is the only field not native to Rosey — it stores the current source text so the connector can detect when the original has changed since the translation was last reviewed. Without it, stale detection is skipped for that entry.
...
For detailed integration patterns — pipeline insertion points, machine translation examples, TMS sync, CI-driven workflows — see [External Integrations](/docs/integrations/).

For AI-powered translation of locale files (using an AI coding agent or custom script), see [AI-Powered Translation](/docs/ai-translation/).
