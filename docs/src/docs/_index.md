...
Run `npx rosey-cloudcannon-connector init --help` for the full list of flags, or see the [init CLI reference](/docs/init/) for what each one does, what the wizard detects, and everything it writes.
...
See [Tagging Content](/docs/tagging-content/) for full details on namespacing and key resolution.
...
See [SSG Setup](/docs/ssg-setup/) for the layout snippet in each generator's template language, including how to derive `data-rosey-root`.
...
The `data-rcc` wrapper includes navigation and footer so their translatable content appears in the locale switcher. If you only need to translate content inside `<main>`, you can omit the wrapper — the connector falls back to `<main>` automatically. See [Configuration: Snapshot boundary](/docs/configuration/#snapshot-boundary) for details.
...
No `install-client` here: this example takes the bundled path above, so Vite pulls the client in at build time. Any generator that doesn't bundle needs the `install-client` step — see [SSG Setup](/docs/ssg-setup/).
...
With `data-rosey-root` on `<main>` in the layout (or on the page itself), the resolved keys would be namespaced further — see [Tagging Content](/docs/tagging-content/) for full details.
