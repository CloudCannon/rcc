...
For production RTL setup (the `dir` script, CSS logical properties, and common overrides), see [RTL Support](/docs/rtl-support/).
...
The `write-locales` CLI validates your config and warns about missing entries — see [write-locales](/docs/write-locales/) for details.
...
> **Tip:** The [`init` wizard](/docs/init/) generates this config for you by default — pass `--no-collection` (or answer "no" to the prompt in interactive mode) if you don't want it.
...
If you need custom toolbar options (e.g. enabling `subscript`, disabling `image`) but your site doesn't use editable regions, [open an issue](https://github.com/CloudCannon/rcc/issues) on the repo and we'll look at adding a configuration path for that.
...
The `write-locales --dest` command generates this file automatically. If you're using your own script instead of `write-locales`, you must produce this manifest yourself — see [write-locales: Using your own script](/docs/write-locales/#using-your-own-script-instead-of-write-locales).
