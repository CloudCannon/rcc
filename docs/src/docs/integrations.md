...
If your workflow doesn't need `write-locales` at all, replace it with your own script. See [write-locales: Using your own script](/docs/write-locales/#using-your-own-script-instead-of-write-locales) for the five requirements the connector expects (flat JSON structure, `original`/`value`/`_base_original` fields, locale manifest, and `data_config` entries).

Keep `npx rosey-cloudcannon-connector install-client` in your postbuild regardless. It's a separate command that copies the browser client into your build output, independent of how locale files get generated — see [SSG Setup](/docs/ssg-setup/).
...
- [write-locales CLI](/docs/write-locales/) — full reference for `write-locales`, including the [contract for custom scripts](/docs/write-locales/#using-your-own-script-instead-of-write-locales)
- [AI-Powered Translation](/docs/ai-translation/) — using AI coding agents and LLMs for translation
- [Incremental Translation](/docs/incremental-translation/) — strategies for rolling out translations progressively
- [Stale Translation Detection](/docs/stale-translations/) — how the `_base_original` field powers stale detection
