...
- Upgrading from v1? → [Migrating from v1](https://rosey.cc/docs/migration-from-v1/)
- New install? → [Getting Started](https://rosey.cc/docs/)
...
- **Stale translation detection** — source text that changed since a translation
  ...
  [Stale Translations](https://rosey.cc/docs/stale-translations/).
...
- **`write-locales` CLI** — reads Rosey's `base.json` and creates/updates locale
  ...
  `--locales` is omitted. See [write-locales](https://rosey.cc/docs/write-locales/).
...
- **Composable postbuild pipeline** — one set of files and one `write-locales`
  ...
  entirely. See [External Integrations](https://rosey.cc/docs/integrations/).
...
- **`tag` CLI and `data-rosey-tagger`** → add `data-rosey` attributes directly.
  See [Tagging Content](https://rosey.cc/docs/tagging-content/).
...
- **Built-in Smartling integration** → no bundled translation service. Any
  ...
  [External Integrations](https://rosey.cc/docs/integrations/).
...
- **Bundled agent skills** → skills are maintained in
  ...
  [AI Translation](https://rosey.cc/docs/ai-translation/).
