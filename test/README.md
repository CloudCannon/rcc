...
- `verify-bundle.mjs` — three things. The installed bundle is byte-identical to the
  repo's `dist/index.mjs` (⇒ local changes under test, not `github:CloudCannon/rcc`;
...
> | `markdown:article.original` in CC's tag-adjacent form | healed onto `_base_original`, translation untouched |
